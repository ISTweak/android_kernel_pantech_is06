...
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
...
#ifndef FEATURE_PANTECH_UMIC_HEADSET//20100811 jhsong
                connType &= ~(1 << 1);
#else
                connType &= ~(1 << 0);
#endif
...
#ifdef CONFIG_SKYSND_USE_EXTAMP
        snd_extamp_api_HeadsetConnected((conn_headset_type != 0));
#endif
        report_headset_switch(hs->ipdev, key, connType);
        printk(KERN_ERR "(skyheadset) Headset Detection(type:0x%x)\n", conn_headset_type);
#else
    ...
    case KEY_VOLUMEUP:
    case KEY_VOLUMEDOWN:
        printk(KERN_ERR "(skyheadset) volume up, down(%d)\n", key_code);
        input_report_key(hs->ipdev, key, (key_code != HS_REL_K));
        break;
#endif
	case -1:
