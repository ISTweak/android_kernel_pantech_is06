...
#endif
{
    ...
    for (y = 0; y < 6; y++) {
        for (x = 0; x < 5; x++) {
            if (data & 1) {
#ifdef SKY_FB_RGB_888   // [DS3] lived 2010.06.08 for support 32bpp (RGBX)
                base[(pixels<<2) - 1] = 0;
            ...
            data >>= 1;
#ifdef SKY_LCD_ROTATE_180    // [DS3] lived 2010.05.19 for 180 degree LCD
            pixels--;
#else
            pixels++;
#endif
...
static void display_log(const char *str, int size)
{
	...
	t_cx = t_cy = 0;
#ifndef SKY_FB_RGB_888   //if (bpp == 2) { //RGB565
    for (k = 0; k < fb_num; k++ ) {
        unsigned short *base;
...
#ifdef SKY_LCD_ROTATE_180    // [DS3] lived 2010.05.19 for 180 degree LCD
            drawglyph((unsigned char *)base, (cmaxy-t_cy) * 12 * fb_width + (cmaxx-t_cx) * 6, FGCOLOR,
    ...
    for (k = 0; k < fb_num; k++ ) {
        ...
        j <<= 2; // [DS3] lived 2010.06.09 for support 32bpp (RGBX)
#endif

        while (count--) {
#ifdef SKY_FRAMEBUFFER_32
            base[--j] = 0;  // [DS3] lived 2010.06.08 for support 32bpp (RGBX)
            base[--j] = (BGCOLOR & 0xFF0000) >> 16;
            base[--j] = (BGCOLOR & 0x00FF00) >> 8;
            base[--j] = (BGCOLOR & 0x0000FF);
#else
    ...
    cx = t_cx; cy = t_cy;
#endif // endof if'n'def SKY_FB_RGB_888	}
    cy++; cx= 0;
}
...
void pantech_errlog_display_with_errlog(bool bmArm, bool do_panic)
{
    ...
    printk("pantech_errlog_display_with_errlog !!!!!!!!!!!!!!!!!!!!\n");
    ...
    if (atomic_read(&is_displayed) == 0) {
        ...
        display_log(pErrorLog, LogIdx);
        display_log("\n\n    Press Power key for USBDUMP\n\n", strlen("\n\n    Press Power key for USBDUMP\n\n"));
		display_log("\n\n    Press VOLDN key for reboot\n\n", strlen("\n\n    Press VOLDN key for reboot\n\n"));
