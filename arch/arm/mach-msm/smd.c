...
static char *crash_buf = NULL;
static int crash_buf_size = 0;

void smd_crash_init(void)
{
	if(crash_buf) return;

	crash_buf = smem_get_entry(SMEM_ERR_CRASH_LOG, &crash_buf_size);
	if (crash_buf != 0) {
		crash_buf[crash_buf_size - 1] = 0;
	}
}
...
void printcrash(const char * fmt, ...)
{
	...
	va_start(args, fmt);
	printed_len += vsnprintf(crash_buf+printed_len, crash_buf_size-printed_len, fmt, args);
	va_end(args);
