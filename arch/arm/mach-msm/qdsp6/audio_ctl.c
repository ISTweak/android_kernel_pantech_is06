		    ...
		    unsigned int cmd, unsigned long arg)
{
	...
	case AUDIO_REINIT_ACDB:
		...
		break;
#ifdef CONFIG_SKYSND_USE_EXTAMP
	case AUDIO_SKY_SET_AMPVOLUME:
		rc = copy_from_user(&n, (void *)arg, sizeof(n));
		snd_extamp_api_SetVolume(n);
		break;
	case AUDIO_SKY_SET_PREAMPGAIN:
		rc = copy_from_user(&n, (void *)arg, sizeof(n));
		snd_extamp_api_SetPreAmpGain(n);
		break;
#endif

#if 1 //def RX_MUTE_IN_CALL //20101022 jhsong
	case AUDIO_PANTECH_SET_RX_MUTE_CALL:
		rc = copy_from_user(&n, (void *)arg, sizeof(n));
		if (!rc)
			rc = q6audio_set_rx_mute_call(n);		
	       printk(KERN_ERR "(q6_ioctl)  kernel : AUDIO_PANTECH_SET_RX_MUTE_CALL  rx_mute : %d,  result : %d\n", n, rc);
		break;
#endif

#ifdef FEATURE_PANTECH_ASR_NEW_DEVICE  //20100823 jhsong
	case AUDIO_PANTECH_GET_CUR_ACDB_ID:
		n = q6audio_get_acdb_id();		
		rc = copy_to_user((void *)arg, &n, sizeof(int));
	       printk(KERN_ERR "(q6_ioctl)  kernel : AUDIO_PANTECH_GET_CUR_ACDB_ID  tx_acdb : 0x%x,  result : %d\n", n, rc);
		break;
#endif
	default:
		rc = -EINVAL;
