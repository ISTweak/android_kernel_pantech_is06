...
#endif
#ifdef CONFIG_SKYSND_USE_ADAPTIVE_VOL_CONTROL
#include "../proc_comm.h"
#endif
#if 0
...
static struct device_vol_table_t device_vol_table[Q6_HW_COUNT] = {
    [Q6_HW_HANDSET] = {
        .level[0] = -1800,
        .level[1] = -1400,
        .level[2] = -1000,
...
#ifdef CONFIG_SKYSND_USE_ADAPTIVE_VOL_CONTROL
static unsigned char voiceOpened = 0;
#endif
...
#ifdef RX_MUTE_IN_CALL  //20101022 jhsong
static int rx_mute = false;
#endif
...
int q6_device_volume(uint32_t device_id, int level)
{
    ...
    for(index = 0; index < MAX_DEVVOL_LV; index++) {
        if (level_table[index] == level)
            break;
    }

    printk("(skyaudio) kernel : device volume = %d, (idx:%d, level:%d)\n", device_vol_table[di->hw].level[index], index, level);
    return device_vol_table[di->hw].level[index];
#else
	struct q6_device_info *di = q6_lookup_device(device_id, 0);
...
#ifdef CONFIG_EF12_BOARD
	for(loop=0; loop<MAX_BUFF_NUM; loop++) {
		if (ac->buf[loop].data){
			iounmap(ac->buf[loop].data);
			pmem_kfree(ac->buf[loop].phys);
		}
	}
#else
	if (ac->buf[0].data) {
...
#ifdef CONFIG_EF12_BOARD
		for(loop=0; loop<MAX_BUFF_NUM; loop++) {
			ac->buf[loop].phys = pmem_kalloc(bufsz,
						PMEM_MEMTYPE_EBI1|PMEM_ALIGNMENT_4K);
			ac->buf[loop].data = ioremap(ac->buf[loop].phys, bufsz);
			if (!ac->buf[loop].data)
				goto fail;

			ac->buf[loop].size = bufsz;
		}
#else
...
static void callback(void *data, int len, void *cookie)
{
	...
	if (e->event_id == ADSP_AUDIO_EVT_STATUS_BUF_DONE) {
		...
		if (ac->flags & AUDIO_FLAG_WRITE) {
			ac->dsp_buf = (++ac->dsp_buf) % MAX_BUFF_NUM;
		}
		else {
			ac->dsp_buf ^= 1;
		}
...
/*EF10S, SBA Patch */
static int enable_bt_dithering(struct audio_client *ac)
{
	struct adsp_audio_set_command set_cmd;

	set_cmd.hdr.opcode = ADSP_AUDIO_IOCTL_CMD_SET;
	set_cmd.control_id = ADSP_AUDIO_AFE_CONTROL;
	set_cmd.param_id = ADSP_AUDIO_PARAM_ID_BT_QCT_DEVICE_ENABLE;
	set_cmd.nlen = 1;
	set_cmd.data[0] = 1;

	return audio_ioctl(ac, &set_cmd,
		sizeof(struct adsp_audio_set_command));
}
...
static void q6_rx_path_enable(int reconf, uint32_t acdb_id)
{
	...
	/* EF10S, SBA Patch */
	if (audio_rx_device_id == ADSP_AUDIO_DEVICE_ID_BT_SCO_SPKR)
		enable_bt_dithering(ac_control);
...
#ifdef CONFIG_SKYSND_USE_ADAPTIVE_VOL_CONTROL
	if(voiceOpened == 1){
		uparam = (unsigned char)q6_device_to_cad_id(audio_rx_device_id) << 16;
		uparam = uparam | (level & 0xFFFF);
		type = 0/*SMEM_PROC_COMM_CUSTOMER_CMD2_SET_AUDIOINFO*/;
		printk(KERN_DEBUG "[SKY Audio] Send audio device information through msm_proc(0x%x)\n", uparam);
		msm_proc_comm(PCOM_CUSTOMER_CMD2, &type, &uparam);
	}
#endif
	return 0;
}
...
#ifdef RX_MUTE_IN_CALL  //20101022 jhsong
int q6audio_set_rx_mute_call(int mute)
{
	uint32_t adev;
	int vol = 0;
	...
	if(mute){
		...
		vol = q6_device_volume(audio_rx_device_id, rx_vol_level);
		audio_rx_volume(ac_control, adev, vol);
	}

	rx_mute = mute;
	printk(KERN_DEBUG "[SKY Audio] q6audio_set_rx_mute_call.............mute : (%d), rx_vol_level : %d, vol : %d\n", mute, rx_vol_level, vol);
#else
	adev = ADSP_AUDIO_DEVICE_ID_VOICE;
	if(mute)
...
int q6audio_do_routing(uint32_t device_id, uint32_t acdb_id)
{
	...
	case Q6_RX:
		...
		case ADSP_AUDIO_DEVICE_ID_SPKR_PHONE_MONO_W_STEREO_HEADSET:
		case ADSP_AUDIO_DEVICE_ID_SPKR_PHONE_STEREO_W_STEREO_HEADSET:
			do_rx_routing(ADSP_AUDIO_DEVICE_ID_SPKR_PHONE_MONO, acdb_id);
			break;
              default:
 			do_rx_routing(device_id, acdb_id);
			break;                    
		}
		snd_extamp_api_SetDevice(device_id);        
#else
		do_rx_routing(device_id, acdb_id);
#endif        
		break;
				      ...
				      uint32_t channels, uint32_t flags, uint32_t acdb_id)
{
...
#ifdef CONFIG_EF12_BOARD  //20110107 jhsong
	no_pcm_write = 0;
#endif
	...
	for (retry = 5;;retry--) {
		...
		if (retry == 0)
#ifdef CONFIG_EF12_BOARD
			printk("[SKY AUDIO  q6audio_open_pcm() 1] Dal timeout! maybe timeout or dsp dead....break\n");
#if 1 //20110106 jhsong
			no_pcm_write = 1;
			break;
#else
	...
	for (retry = 5;;retry--) {
		...
		if (retry == 0)
#ifdef CONFIG_EF12_BOARD
			printk("[SKY AUDIO q6audio_open_pcm() 2] Dal timeout! maybe timeout or dsp dead....break\n");
#if 1 //20110106 jhsong
			no_pcm_write = 2;
			break;
#else
...
#ifdef FEATURE_PANTECH_ASR_NEW_DEVICE  //20100823 jhsong
uint32_t q6audio_get_acdb_id(void)
{
	return tx_acdb;
}
#endif
