			   ...
			   size_t count, loff_t *pos)
{
	...
	ac = pcm->ac;
#if  1//20110108 jhsong
	if (!ac){
		printk("[SKY AUDIO pcm_write()] ac pointer null so return -ENODEV ... ac : 0x%x.\n", ac);
		return -ENODEV;
	}
#else
	if (!ac)
	...
	while (count > 0) {
...
#ifdef CONFIG_EF12_BOARD  //20110104 jhsong : reset in pcm_write, so ab null pointer check
		if (ab == NULL){
			printk("[SKY AUDIO pcm_write()] ab pointer null so return -ENODEV ... ab : 0x%x, ac : 0x%x.\n", ab, ac);
			return -ENODEV;
		}
#endif
		...
		ab->used = 1;
		ab->actual_size = xfer;
#if 1 //def CONFIG_EF12_BOARD	//20110108 jhsong	
    if(!no_pcm_write)
    {
		q6audio_write(ac, ab);
    }else{
		printk("[pcm_write()] open pcm -EIO error........so not pcm write....no_pcm_write : %d.\n", no_pcm_write);    
    }
#else
	q6audio_write(ac, ab);
#endif
...
#ifdef CONFIG_EF12_BOARD
		ac->cpu_buf = (++ac->cpu_buf) % MAX_BUFF_NUM;
#else
		ac->cpu_buf ^= 1;
#endif
	}
