// shpark 091229 add by IMMERSION support
// shpark 100217 modify by IMERSION support
// --------------------------------------
...
// shpark 100217 edit : tspdrv is not MODULE but Kernel Object
#if (MODEL_ID == MODEL_EF12)
#else
#ifndef MODULE
#define MODULE
#endif
...
static struct early_suspend tspdrv_es;

static int  tspdrv_early_suspend(struct work_struct *work)
{
	#if (MODEL_ID == MODEL_EF12)
	motor_disable();
	g_bIsPlaying = false;
...
static int  tspdrv_late_resume(struct work_struct *work)
{
	#if (MODEL_ID == MODEL_EF12)
	//motor_enable(); // shpark 100301
	#else
...
// shpark 100217 edit : tspdrv is not MODULE but Kernel Object
#if (MODEL_ID == MODEL_EF12)
static int __init tspdrv_init_module(void)
#else
...
#endif
{
	...
	#ifdef CONFIG_HAS_EARLYSUSPEND
	tspdrv_es.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN;
	tspdrv_es.suspend = tspdrv_early_suspend;
	tspdrv_es.resume = tspdrv_late_resume;
	register_early_suspend(&tspdrv_es);
	#endif
...
// shpark 100217 edit : tspdrv is not MODULE but Kernel Object
#if (MODEL_ID == MODEL_EF12)
static void __exit tspdrv_cleanup_module(void)
#else
...
static ssize_t write(struct file *file, const char *buf, size_t count, loff_t *ppos)
{
    ...
    //DbgOut("[SKY MOTOR][ tspdrv : write] count = %d\n", count);
...
#error "SPI_HEADER_SIZE expected to be 3"
    ...
    DbgOut("[SKY MOTOR][ tspdrv : write_RUN_VibeOSKernelLinuxStartTimer]\n");
...
static int ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	//DbgOut("[SKY MOTOR][ tspdrv : ioctl cmd(%d), arg(%d)]\n",cmd, arg);
    switch (cmd)
    {
        case TSPDRV_STOP_KERNEL_TIMER:
//DbgOut("[ tspdrv : ioctl_TSPDRV_STOP_KERNEL_TIMER g_blsPlaying=%d]\n", g_bIsPlaying);
            /* 
...
// shpark 091229 edit : tspdrv is not MODULE but Kernel Object
#if (MODEL_ID == MODEL_EF12)
module_init(tspdrv_init_module);
module_exit(tspdrv_cleanup_module);
#endif
