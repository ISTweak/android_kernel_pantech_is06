...
#define FEATURE_PANTECH_PMIC_CHG // BM_System Added for Pantech PMIC Charging.
...
#define BATTERY_READ_STATUS_PROC		31
#endif // #if defined(FEATURE_PANTECH_PMIC_CHG)
...
#else // FEATURE_PANTECH_PMIC_CHG
#define BATTERY_LOW            	2800
#define BATTERY_HIGH           	4300
#endif // FEATURE_PANTECH_PMIC_CHG
...
static int msm_batt_get_batt_chg_status(void)
{
	...
	return 0;
	#else // #if defined(FEATURE_PANTECH_PMIC_CHG)
	int rc ;
...
static void msm_batt_update_psy_status(void)
{
  ...
  u32 battery_temp;
	...
	{
	  pr_err( "%s : RETURN. To get battery charging info returned error \n", __func__ );
	  return;
	}

	charger_status        = rsp.charger_status;
	charger_hardware_type = rsp.charger_hardware_type;
  ...
  if (charger_status == msm_batt_info.charger_status &&
	    charger_hardware_type == msm_batt_info.charger_type &&
  ...
  unnecessary_event_count = 0;
	...
	pr_info("BATT Info: rcvd: CS:%d, CT:%d, BS:%d,BL:%d; BV:%d, BT:%d\n",
		 charger_status, charger_hardware_type, battery_status,
  ...
  if (battery_status == BATTERY_STATUS_INVALID &&
		...
		if (charger_hardware_type == CHARGER_TYPE_USB_WALL ||
		    charger_hardware_type == CHARGER_TYPE_USB_PC ||
		    charger_hardware_type == CHARGER_TYPE_USB_CARKIT) {
			pr_info("BATT: USB charger plugged in\n");
			msm_batt_info.current_chg_source = USB_CHG;
			supp = &msm_psy_usb;
		} else if (charger_hardware_type == CHARGER_TYPE_WALL) {
        ...
        else
        {
          msm_batt_info.batt_status =	POWER_SUPPLY_STATUS_CHARGING;
				...
				if (msm_batt_info.current_chg_source & AC_CHG) {
          if ((charger_status == CHARGER_STATUS_WEAK) ||
              (charger_status == CHARGER_STATUS_BAD) ||
              (battery_status == BATTERY_STATUS_BAD_TEMP)) {
            pr_info("BATT: charger_status:%d, battery_status:%d\n",
              charger_status, battery_status);
            pr_info("BATT: AC but discharging.\n");
            msm_batt_info.batt_status = POWER_SUPPLY_STATUS_DISCHARGING;
          }
          
/*
          pr_info("BATT Info: batt_status:%d, battery_status:%d\n",
		 msm_batt_info.batt_status, battery_status);
          if ((msm_batt_info.batt_status == POWER_SUPPLY_STATUS_CHARGING) &&
              (battery_status == BATTERY_STATUS_BAD_TEMP)) {
              pr_info("BATT: Overheat Discharging.\n");
              msm_batt_info.batt_status = POWER_SUPPLY_STATUS_DISCHARGING;
          } */
					supp = &msm_psy_ac;
        }
...
//	}
  ...
  msm_batt_info.batt_capacity   = battery_soc;
	...
	msm_batt_info.charger_type 	  = charger_hardware_type;
