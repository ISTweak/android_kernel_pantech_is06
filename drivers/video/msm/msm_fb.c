...
#ifdef CONFIG_SKY_HDMI_SIL9024A
...
static int clk_setting_first = 0;   // lived 20091120
//static int lcdc_path_setting = 0;   // lived 20091120
//static int tl2796_screen_status = 0;
#endif // end of CONFIG_FB_MSM_LCDC
...
#ifdef CONFIG_SKY_HDMI_SIL9024A
static int msm_fb_output_switch(struct msm_fb_data_type *mfd);
#endif
#ifdef CONFIG_EF12_BOARD
static int msm_fb_sky_ioctl(struct fb_info *info, unsigned int cmd,
        unsigned long arg);
#endif
...
// [DS3] lived, 2010.06.30 SW Reset for double buffering
...
#endif
            tl2796_screen_off(); //LeeCY.. for black screen before H/W init
#endif
...
#ifdef CONFIG_EF12_BOARD
#ifdef CONFIG_SKY_HDMI_SIL9024A
            int lcdc_width = mfd->panel_info.xres;
            int lcdc_height = mfd->panel_info.yres;
            if (lcdc_width == 480 && lcdc_height == 800) {
#endif //CONFIG_SKY_HDMI_SIL9024A
                //mdelay(50);
                msleep(60);
#ifdef CONFIG_SKY_HDMI_SIL9024A
            }
...
*/
	...
	default:
        ...
        if (mfd->panel_power_on) {
            ...
            //20101012 jjangu_device 504005 patch not apply
            //msleep(16);
            mdelay(100);
...
static struct fb_ops msm_fb_ops = {
	...
	.fb_mmap = msm_fb_mmap,
#ifdef CONFIG_EF12_BOARD
    .fb_sky_ioctl = msm_fb_sky_ioctl,   /* SKY IOCTL */
#endif
};
...
static int msm_fb_register(struct msm_fb_data_type *mfd)
{
	...
	int *id;
	int fbram_offset;
#ifdef SKY_FB_HDMI_16BPP_RGB_565
	uint32 fb_imgType;
#endif
...
#ifdef SKY_FB_HDMI_16BPP_RGB_565
	/* lived 20091204 AM-OLED is RGB_888, HDMI is RGB_565 */
	if ((panel_info->xres == 1280 && panel_info->yres == 720)
			|| (panel_info->xres == 720 && panel_info->yres == 480))
		fb_imgType = MDP_RGB_565;
	else
		fb_imgType = mfd->fb_imgType;
	switch (fb_imgType) {
#else
...
//20100907 jjangu_device for the KDDI
#if 0
			      ...
			      struct fb_info *info)
{
	...
	// [DS3] lived, 2010.06.30 SW Reset for double buffering
	fb_offset = info->var.yoffset * info->fix.line_length;
...
static int msm_fb_output_switch(struct msm_fb_data_type *mfd)
{
    ...
    if((lcdc_width == 720) && (lcdc_height == 480))
    {
        data_en_polarity = 0;
        vsync_polarity = 1;
        hsync_polarity = 1;
        printk("%s 720, 480 polarity\n", __func__);
    }
    else
    {
//20100809 jjangu_device
    ...
    //printk(KERN_ERR "hsync=%d, vsync=%d\n", hsync_ctrl, vsync_period);
        ...
        unsigned long arg)
{
    ...
    int ret = 0;
    unsigned int dev = 0;
    unsigned long pclk = 24576 * 1000 - 500;

    switch (cmd) {
    case FBIO_SKY_INITIALIZE_LCD:
        printk(KERN_DEBUG "[LIVED] FBIO_SKY_INITIALIZE_LCD cmd=%d\n", cmd);
...
#ifdef CONFIG_SKY_HDMI_SIL9024A
    case FBIO_SKY_SWITCH_OUTPUT:
        printk(KERN_DEBUG "[LIVED] FBIO_SKY_SWITCH_OUTPUT cmd=%d\n", cmd);
        //down(&msm_fb_ioctl_ppp_sem);
        ret = copy_from_user(&dev, argp, sizeof(dev));
        ...
        if (dev == 1)
            ...
            pclk = 27027 * 1000 - 500;//25175 * 1000 - 500;//27027 * 1000 - 500;

        printk(KERN_DEBUG "sky_hdmi_switch(%d)\n", dev);
        printk(KERN_DEBUG "sky_hdmi_switch(%d) pclk=%ld\n", dev, pclk);
        sky_hdmi_switch(dev);
        ret = msm_fb_output_switch(mfd);
        // PCLK setting
        if (clk_setting_first == 0)
        {
        ...
        clk_set_rate(mdp_lcdc_pclk_clk, pclk);
        clk_set_rate(mdp_lcdc_pad_pclk_clk, pclk);
        printk(KERN_DEBUG "[LIVED] mdp_lcdc_pclk_clk=%ld\n", 
                clk_get_rate(mdp_lcdc_pclk_clk));
        printk(KERN_DEBUG "[LIVED] mdp_lcdc_pad_pclk_clk=%ld\n", 
                clk_get_rate(mdp_lcdc_pad_pclk_clk));

        //up(&msm_fb_ioctl_ppp_sem);
        break;

    case FBIO_SKY_SCREEN_ONOFF:
        printk(KERN_DEBUG "[LIVED] FBIO_SKY_SCREEN_ONOFF cmd=%d\n", cmd);
        ret = copy_from_user(&dev, argp, sizeof(dev));
        switch (dev)
        {
        case 0:// OFF
            tl2796_screen_off();
            //tl2796_screen_status = 0;
            break;
        case 1:// ON
            tl2796_screen_on();
            //tl2796_screen_status = 1;
            break;
        default:
            ret = -EINVAL;
            break;
    ...
    case FBIO_SKY_HDMI_ENABLE:
        printk(KERN_DEBUG "[LIVED] FBIO_SKY_HDMI_ENABLE cmd=%d\n", cmd);
        ret = copy_from_user(&dev, argp, sizeof(dev));
