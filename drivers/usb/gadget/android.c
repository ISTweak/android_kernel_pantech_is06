...
#if defined(CONFIG_ANDROID_PANTECH_HSUSB) && defined(FEATURE_AARM_RELEASE_MODE)
...
// 20100920 YGP for Pantech USB Driver porting ========>
#if defined(CONFIG_ANDROID_PANTECH_HSUSB)
extern int pantech_whcm_function_add(struct usb_composite_dev *cdev, struct usb_configuration *c);
extern int pantech_obex_function_init(void);
extern int pantech_obex_function_add(struct usb_composite_dev *cdev, struct usb_configuration *c);
#endif
// 20100920 YGP for Pantech USB Driver porting ========<
...
// 20100920 YGP for Pantech USB Driver porting ========>
#if defined(CONFIG_ANDROID_PANTECH_HSUSB)
android_func_attr(pantech_whcm, ANDROID_PANTECH_WHCM);
android_func_attr(pantech_obex, ANDROID_PANTECH_OBEX);
#endif
// 20100920 YGP for Pantech USB Driver porting ========<
...
// 20100920 YGP for Pantech USB Driver porting ========>
#if defined(CONFIG_ANDROID_PANTECH_HSUSB)
	&dev_attr_pantech_whcm.attr,
	&dev_attr_pantech_obex.attr,
#endif
// 20100920 YGP for Pantech USB Driver porting ========<
...
static int android_pantech_set_serial_number(void)
{
  ...
  if(*ret.out_len <= 0)
  {
	...
	memcpy(pantech_serial_number, SKY_MODEL_NAME, strlen(SKY_MODEL_NAME));
...
// 20100920 YGP for Pantech USB Driver porting ========>
			...
			if(device_desc.idVendor == PANTECH_VENDOR_ID){
				ret = diag_function_add(c, pantech_serial_number);
			} else {
...
// 20100920 YGP for Pantech USB Driver porting ========>
#if defined(CONFIG_ANDROID_PANTECH_HSUSB)
		case ANDROID_PANTECH_WHCM:
			ret = pantech_whcm_function_add(dev->cdev, c);
			if (ret) {
				printk(KERN_ERR "failed to add whcm function\n");
			return ret;
		}
			break;
		case ANDROID_PANTECH_OBEX:
			ret = pantech_obex_function_add(dev->cdev, c);
			if (ret){
				printk(KERN_ERR "failed to add obex function\n");
	                return ret;
                 }
			break;
#endif
...
static int is_iad_enabled(void)
{
	...
	while (n) {
		...
		case ANDROID_ACM_NMEA:
#ifdef CONFIG_USB_ANDROID_RNDIS
    #ifndef CONFIG_ANDROID_PANTECH_HSUSB
		case ANDROID_RNDIS:
    #endif
#endif
			return 1;
...
// 20100920 YGP for Pantech USB Driver porting ========>
#if defined(CONFIG_ANDROID_PANTECH_HSUSB) 
	ret = pantech_obex_function_init();
	if(ret)
		goto free_dev;
#endif
// 20100920 YGP for Pantech USB Driver porting ========<
