...
/* do not use nmea, only modem interface */
...
#if defined(CONFIG_HSUSB_SKY) || defined(CONFIG_HSUSB_PANTECH_OBEX)
	struct usb_interface_descriptor gs_ifc_cdc_desc;
	struct usb_interface_descriptor gs_ifc_acm_desc;
#else
...
#if defined(CONFIG_HSUSB_PANTECH_OBEX)
static void gs_init_ifc_cdc_desc(struct usb_interface_descriptor *ifc_desc)
{
	ifc_desc->bLength =		USB_DT_INTERFACE_SIZE;
	ifc_desc->bDescriptorType =	USB_DT_INTERFACE;
	ifc_desc->bNumEndpoints =	1;
	ifc_desc->bInterfaceClass =	0x02;
	ifc_desc->bInterfaceSubClass = 0x02;	
	ifc_desc->bInterfaceProtocol = 0x01;	
	ifc_desc->iInterface =		0;
};

static void gs_init_ifc_acm_desc(struct usb_interface_descriptor *ifc_desc)
{
	ifc_desc->bLength =		USB_DT_INTERFACE_SIZE;
	ifc_desc->bDescriptorType =	USB_DT_INTERFACE;
	ifc_desc->bNumEndpoints =	2;
	ifc_desc->bInterfaceClass =	0x0A;
	ifc_desc->bInterfaceSubClass = 0x00;	
	ifc_desc->bInterfaceProtocol = 0x00;	
	ifc_desc->iInterface =		0;
};

static void gs_init_ifc_obex_cdc_desc(struct usb_interface_descriptor *ifc_desc)
{
	ifc_desc->bLength =		USB_DT_INTERFACE_SIZE;
	ifc_desc->bDescriptorType =	USB_DT_INTERFACE;
	ifc_desc->bNumEndpoints =	0;
	ifc_desc->bInterfaceClass =	0x02;
	ifc_desc->bInterfaceSubClass = 0x0B;	
	ifc_desc->bInterfaceProtocol = 0x00;	
	ifc_desc->iInterface =		0;
};

static void gs_init_ifc_obex_acm_desc(struct usb_interface_descriptor *ifc_desc)
{
	ifc_desc->bLength =		USB_DT_INTERFACE_SIZE;
	ifc_desc->bDescriptorType =	USB_DT_INTERFACE;
	ifc_desc->bNumEndpoints =	2;
...
static void gs_init_header_desc(struct gs_dev *dev)
{
	...
	dev->gs_highspeed_header[5] = NULL;
	...
	dev->gs_fullspeed_header[5] = NULL;
}

static void gs_init_obex_header_desc(struct gs_dev *dev)
{
	dev->gs_highspeed_header[0] =
		(struct usb_descriptor_header *)&dev->gs_ifc_cdc_desc;
	dev->gs_highspeed_header[1] =
...
static int __init gs_module_init(void)
{
	...
	for (i = 0; i < instances; i++) {
		...
		if(strcmp(a[i], "obex") == 0){
			...
			/*2 - Interface(cdc,acm), 3 Endpoints-> Total 5 + 1 for NULL*/
			gs_devices[i]->gs_fullspeed_header =
			...
			/* cdc interface & endpoint */
			gs_init_ifc_cdc_desc(&gs_devices[i]->gs_ifc_cdc_desc);
			gs_init_ep_desc(&gs_devices[i]->gs_hs_notifyin_desc, INTERRUPT,
					HIGHSPEED);

			/* acm interface & endpoints */
			gs_init_ifc_acm_desc(&gs_devices[i]->gs_ifc_acm_desc);
			gs_init_ep_desc(&gs_devices[i]->gs_hs_bulkin_desc, BULK,
					HIGHSPEED);
			gs_init_ep_desc(&gs_devices[i]->gs_hs_bulkout_desc, BULK,
					HIGHSPEED);
...
#ifdef CONFIG_HSUSB_PANTECH_OBEX
	if(ep == NULL) {
		printk(KERN_ERR "endpoint is NULL: maybe obex\n");
		return;
	}
#endif
...
static void gs_bind(void *_ctxt)
{
	...
	dev->gs_ifc_cdc_desc.bInterfaceNumber = ret;
	dev->gs_ifc_cdc_desc.iInterface = 0;

	ret = usb_msm_get_next_ifc_number(func);
	dev->gs_ifc_acm_desc.bInterfaceNumber = ret;
	dev->gs_ifc_acm_desc.iInterface = 0;
#else
	ret = usb_msm_get_next_ifc_number(func);
...
#ifdef CONFIG_HSUSB_PANTECH_OBEX
	if(func && (func->name != a[OBEX_INDEX])) {
