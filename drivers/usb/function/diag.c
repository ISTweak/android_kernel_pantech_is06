		...
		struct usb_request *req)
{
	...
	if((pantech_usb_test_get_run() == 1) 
	  ...
	  && (pantech_usb_test_get_port_type() == 1))
	{
		//printk("diag_loopback_read buf_size[%d] = count[%d]\n", req->length, req->actual);

		if(req->status == 0) { 
			if(req->actual == 0) { 
			  pantech_usb_test_send_loopback((void*)ctxt, ctxt->epin);
		...
		usb_ept_queue_xfer(ctxt->epout, req);
