...
#include <mach/clk.h>
#if defined(CONFIG_HSUSB_PANTECH_OBEX) || defined(CONFIG_HSUSB_SKY)
...
static usb_test_cmd_type loopback_usb_test_cmd;
...
static int pid = 0x1103;
#elif defined(CONFIG_HSUSB_SKY)
static int pid = 0x1102;
#else
...
#if defined(CONFIG_HSUSB_PANTECH_OBEX) || defined(CONFIG_HSUSB_SKY)
static char adb_serial_number[20]; //EF10S + BSN(14) + 1
static int msm_hsusb_set_serial_number(char **serial_number)
{
	...
	memset(adb_serial_number, 0x00, 20);

	client = oem_rapi_client_init();
	if(IS_ERR(client))
	{
		printk(KERN_ERR "hsusb set pdata error\n");
		memcpy(adb_serial_number, "EF10SXXXXXXXXXXXXXX", 19);
		*serial_number = adb_serial_number;
		return 0;
	}
	...
	if(*ret.out_len <= 0)
	{
		printk(KERN_ERR "hsusb set pdata error:output[%d]\n", *ret.out_len);
		memcpy(adb_serial_number, "EF10S00000000000000", 19);
		*serial_number = adb_serial_number;
		oem_rapi_client_close();
		return 0;
	}
	
	memcpy(adb_serial_number, "EF10S", strlen("EF10S"));
	memcpy(adb_serial_number + 5, ret.output, *ret.out_len);
	*serial_number = adb_serial_number;
	oem_rapi_client_close();
...
static void pantech_usb_test_null(struct usb_endpoint *ept, struct usb_request *req)
{
	//printk("sended 0 length packed\n");
}

static void pantech_usb_test_loopback_in_complete(struct usb_endpoint *ept, struct usb_request *req)
{
	if(req->status == 0)
	...
	if(loopback_buffer_sended_bulk_count >= loopback_buffer_bulk_count) {
		...
		if((loopback_buffer_send_count % ept->max_pkt) == 0) {
			loopback_usb_request->complete = pantech_usb_test_null;
			loopback_usb_request->length = 0;
			usb_ept_queue_xfer(ept, loopback_usb_request);
		}
...
void pantech_usb_test_send_loopback(void *context, struct usb_endpoint *ept)
{
...
static void pantech_usb_test_bind_loopback(struct usb_info *ui)
{
	if(!ui) return;

	if(!loopback_usb_request) {
		loopback_usb_request = usb_ept_alloc_req(&ui->ep0in, 0);
		if(!loopback_usb_request) {
...
#ifdef SKY_PROCESS
	if(!ui->pdata->serial_number) {
		msm_hsusb_set_serial_number(&ui->pdata->serial_number);
	}
#endif
#endif
...
#if defined(CONFIG_HSUSB_PANTECH_USB_TEST)
	pantech_usb_test_bind_loopback(ui);
#endif
