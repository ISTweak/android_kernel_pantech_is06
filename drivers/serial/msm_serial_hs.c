...
#ifdef CONFIG_UART2DM_CONSOLE
#define is_hs_console(uport)	((uport)->cons && \
				(uport)->cons->index == (uport)->line)

static void msm_hs_console_putchar(struct uart_port *uport, const char* c, unsigned int tx_count);
#endif // #ifdef CONFIG_UART2DM_CONSOLE
...
static void msm_hs_submit_tx_locked(struct uart_port *uport)
{
  ...
  else
  {
    /* Save tx_count to use in Callback */
    tx->tx_count = tx_count;
    msm_hs_console_putchar(uport, &tx_buf->buf[tx_buf->tail], tx_count);
  }
#endif // #ifdef CONFIG_UART2DM_CONSOLE
}

/// wjmin 2010.5.6@DS2 for root shell
#ifdef CONFIG_UART2DM_CONSOLE
...
#else
#define msm_hs_suspend NULL
#define msm_hs_resume NULL
#endif
...
static void msm_hs_reset(struct uart_port *port)
{
	/* reset everything */
	msm_hs_write(port, UARTDM_CR_ADDR, RESET_RX);
	msm_hs_write(port, UARTDM_CR_ADDR, RESET_TX);
	msm_hs_write(port, UARTDM_CR_ADDR, RESET_ERROR_STATUS);
	msm_hs_write(port, UARTDM_CR_ADDR, RESET_BREAK_INT);
	msm_hs_write(port, UARTDM_CR_ADDR, RESET_CTS);
	msm_hs_write(port, UARTDM_CR_ADDR, RFR_HIGH);
}
...
static void msm_hs_console_putchar(struct uart_port *uport, const char* c, unsigned int tx_count)
{
  ...
  while( left > 0)
  {
    ...
    msm_hs_write(uport, UARTDM_NCF_TX_ADDR, i);
    msm_hs_write(uport, UARTDM_CR_ADDR, CLEAR_TX_READY);
    msm_hs_write(uport, UARTDM_TF_ADDR, tx);
    msm_uport->imr_reg |= UARTDM_ISR_TX_READY_BMSK;
    msm_hs_write(uport, UARTDM_IMR_ADDR, msm_uport->imr_reg);
			      ...
			      unsigned int count)
{
  ...
  uport = &msm_uport->uport;
	...
	msm_hs_write(uport, UARTDM_CR_ADDR, UARTDM_CR_TX_EN_BMSK);
	msm_hs_console_putchar(uport, s, count);
	clk_disable(msm_uport->clk);
  spin_unlock_irqrestore(&uport->lock, flags);
...
static int __init msm_hs_console_setup(struct console *co, char *options)
{
  ...
  uport = &msm_uport->uport;
	...
	parity = 'n';
	flow = 'n';
//	msm_hs_write(uport, UARTDM_MR2_ADDR, UARTDM_MR2_BITS_PER_CHAR_BMSK | UARTDM_MR2_STOP_BIT_LEN_BMSK);	/* 8N1 */
